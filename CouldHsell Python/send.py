...
try:
    ...
    import queue
    import threading
    import os,codecs,string,re,time,ssl
...
def findString(email,xstring):
    ...
    xstring = xstring.replace("{amount}",RandomNumber(1)+","+RandomNumber(3))
    ...
    xstring = xstring.replace("{tld}", get_tld(email, fix_protocol=True))
    xstring = xstring.replace("[tld]", get_tld(email, fix_protocol=True))
    xstring = xstring.replace("[frmsite]", domain_name)
    ...
    if UseGoogle:
        GoogleApi="hqg407u344scz7fo9ehg1cxaxkom2i"
        urls = re.findall('<a\s+(?:[^>]*?\s+)?href="([^"]*)"', xstring)

        for turl in urls:
            ##print(link)
            if turl.startswith('http') and turl != None:
                i = 0
                nurl = str(GoogleUrl(turl,GoogleApi))
                while 'no redirects' in nurl and nurl == None and '403 Forbidden' in nurl:
                    if i == 3:
                        break
                    i += 1
                if not 'no redirects' in nurl and nurl != None and not '403 Forbidden' in nurl and itsurl(nurl):
                    xstring = xstring.replace(turl,nurl)
    return str(xstring)
def itsurl(link):
    regex = re.compile(
...
def Screnshot(email):
    ...
    try:
        ...
        hti.screenshot(html_str=html, css_str=css,  save_as=imagename, size=(1240, 3508))
...
def Sendout(emails,smtpserv,port,its_authy,username,password,fromemail,sender,its_secure):
    global texto,subject,name,sfile,link
    if '{mx}' in smtpserv:
        mxRecord = ''
        try:
    ...
    corpo = MIMEText(nmsg, 'html', 'utf-8')
